...
`std::string` е клас от стандартната библиотека на C++, който се използва за работа със символни низове (текст). Можете да мислите за него като ООП еквивалента на базовия символен низ (`const char *`) или като за `std::vector<char>`.
